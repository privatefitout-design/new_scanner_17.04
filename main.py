...
def calculate_score(data_history):
    ...
    if price_range_pct < 1.3:   score += 5
    elif price_range_pct < 2.5: score += 3

    if 5 <= oi_growth_pct <= 18:   score += 6
    elif 4 <= oi_growth_pct <= 25: score += 3

    if current_oi > 100_000_000: score += 4
    elif current_oi > 50_000_000:  score += 2

    if recent_change < 1.0:   score += 3
...
async def process_symbol(client, symbol):
    try:
        ...
        if symbol not in history:
            history[symbol] = deque(maxlen=80)
...
📉 Диапазон цены: <code>{price_range_pct:.2f}%</code>
⏰ {now.strftime('%H:%M')}
                """.strip()

                await send_telegram(alert)
                log.info(f"СИГНАЛ [{score:.1f}/10 | {base_days} дней] → {symbol}")
...
async def main():
    log.info(f"Scanner запущен | Мин. база: {MIN_BASE_DAYS} дней | Скан каждые {SCAN_INTERVAL_MINUTES} минут")
